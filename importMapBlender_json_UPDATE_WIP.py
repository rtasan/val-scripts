...
def getFixedPath(objectProperties):
    a = CWD.joinpath("export", os.path.splitext(objectProperties["StaticMesh"]["OuterIndex"]["ObjectName"])[0].strip("/")).__str__()
...
def setMaterial(byoMAT: bpy.types.Material, matJSON: list, override: bool = False):
    ...
    def setupTexture(byoMAT, index: int, matJSON: str) -> bpy.types.ShaderNodeTexImage:
        texturePath = matJSON[index+1].replace(" ", "").replace("ParameterValue=", "").replace("Texture2D", "").replace("'", "")
    ...
    def setupVector(index: int, matJSON: str) -> str:
        a = matJSON[index+1].replace(" ", "").replace("ParameterValue=", "").replace("{", "").replace("}", "")
        a = a.replace("R=", "").replace("G=", "").replace("B=", "").replace("A=", "")
        a = a.split(",")
    ...
    for index, line in enumerate(matJSON):
        ...
        if "ParameterInfo = { Name=DiffuseColor }" in line:
            Diffuse_Color.outputs[0].default_value = setupVector(index, matJSON)
        ...
        if "ParameterInfo = { Name=Emissive Mult }" in line:
            Emissive_Mult.outputs[0].default_value = setupVector(index, matJSON)
            Emissive_Mult.use_custom_color = True
            Emissive_Mult.color = usedColor

        if "ParameterInfo = { Name=Emissive Color }" in line:
            Emissive_Color.outputs[0].default_value = setupVector(index, matJSON)
        ...
        if "ParameterInfo = { Name=Min Light Brightness Color }" in line:
            ML_Brightness.outputs[0].default_value = setupVector(index, matJSON)
        ...
        if "    OpacityMaskClipValue = " in line:
            byoMAT.alpha_threshold = float(line.replace(" ", "").replace("OpacityMaskClipValue=", ""))
...
def importObject(object, objectIndex, umapName, mainScene, provider: Provider):
    objectProperties = object["ExportValue"]
    ...
    umapPKG = provider.get_package(getObjectPath(objectProperties))
    # umapPKG = provider.get_package(getObjectname(objectProperties))
    umapPKG = umapPKG.save_json("yo")


    # logger.warning(objJSON)
        ...
        if umapPKG is not None:
            ...
            for objectIndex, object in enumerate(umapDATA):
                if checkImportable(object):
                    importObject(object, objectIndex, umapName, main_scene, provider)
